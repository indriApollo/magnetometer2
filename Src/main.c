...
#include "graphics.h"
#include "uart.h"
// offset against the real north
#define HEADING_OFFSET 166
// we have to be sure we have enough samples to cover an entire sphere
// else the calibration will fail
#define CALIB_N_SAMPLES 400

#define LCD_WIDTH 480
#define LCD_HEIGHT 272
#define RGB565_IMGSIZE (LCD_WIDTH*LCD_HEIGHT*2)

#define FB1 0xc0000000
#define FB2 (FB1+RGB565_IMGSIZE)
...
uint16_t arrowTmpBuf[272*272];
uint16_t calib_loadingTmpBuf[90*90];
uint16_t fontBuffer[FONT_SIZE*N_CHARS];
uint16_t fontBufferRtd[FONT_SIZE*N_CHARS];
...
int main(void)
{
	...
	HAL_Delay(100); // ample startup time

	float rad, deg, roll, pitch, rollCos, rollSin, pitchCos, pitchSin;
	float p = 0.125f; // moving average weight: 1/8
	float degprev = 0;
	char msg[64];
	int16_t heading;
	sample caliMin;
...
static void initScreen1(void)
{
	clearBuffer((uint16_t*)FB1, RGB565_IMGSIZE/2, 0x0000);
...
static void updateScreen1(float rad, int16_t deg)
{
	...
	cpyRotated(fontBuffer, fontBufferRtd, 150, 74);
	cpyToFb(fontBufferRtd, 74, 150, (uint16_t*)FB1, LCD_WIDTH, 400, 61);
	rotateImage(rad, arrow, arrowTmpBuf, 272, 272);
	cpyToFb(arrowTmpBuf, 272, 272, (uint16_t*)FB1, LCD_WIDTH, 100, 0);
}

static void updateScreen2(void)
{
	rotateImage(loadingRad, calib_loading, calib_loadingTmpBuf, 90, 90);
	cpyToFb(calib_loadingTmpBuf, 90, 90, (uint16_t*)FB2, LCD_WIDTH, 20, 91);
	loadingRad += 0.31415926536f; // 18 deg
}
